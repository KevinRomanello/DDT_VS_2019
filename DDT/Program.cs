...
namespace DDTImport
{
    ...
    public class DR_Contab_ImportDDT
    {
        ...
        private void VerificaDestinatario(DocumentoToImport documento)
        {
            ...
            if (documento.Cliente != null && documento.Cliente.IndexOf("ERRE-BI", StringComparison.OrdinalIgnoreCase) >= 0)
            {
        ...
        private DocumentoToImport ReadDDT_from_Wurth(string text)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                try
                {
                    ...
                    // potrebebro essere salvati in CodiceCliente
                    ...
                    // potrebbe essere utile salvarlo
                    var valore14 = values[columnIndexes["TOTALE_IVA"]].Trim();                      // Valore non salvato

                    // esiste gia' un valore simile 
                    var valore15 = values[columnIndexes["DATA_ORDINE"]].Trim();                     // Valore non salvato
                    ...
                    if (documento.DocNumero == null)
                    {
                        ...
                        documento.DestinazioneMerce1 = $"{via}\n{cap} {citta} {paese} ({provincia})";
                        documento.Note = values[columnIndexes["NOTA"]].Trim();
        ...
        private decimal? ParseNullableDecimal(string value)
        {
            ...
            decimal result;
            if (decimal.TryParse(value, NumberStyles.Any, CultureInfo.GetCultureInfo("it-IT"), out result))
                return result;
        ...
        private DocumentoToImport ReadDDT_from_SVAI(string text)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                try
                {
                    ...
                    // Formatta l'indirizzo correttamente
                    var via = fields[columnIndexes["Indirizzo"]].Trim();
                    var cap = fields[columnIndexes["CAP"]].Trim();
                    var provincia = fields[columnIndexes["Provincia"]].Trim();
                    var paese = fields[columnIndexes["Localita"]].Trim();
                    documento.DestinazioneMerce1 = $"{via}\n{cap} {paese} ({provincia})";
